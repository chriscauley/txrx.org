...
function setTime() {
  ...
  function n(t) { return ('0'+t).slice(-2) }
  $(".countdown").text(n(h)+":"+n(m)+":"+n(s));
  if (s == 0) {
    $(".times").css({left:-Math.round((now-window.t0)/1000/3600*82)-time_slip});
  }
}
...
function cboxThis(that) {
  function title() { return $(this).text() }
  $(that).children('a').colorbox({rel:'gal',open:true, title: title});
}