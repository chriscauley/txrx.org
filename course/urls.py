...
from views import instructor, evaluation, debug_parsing, classes, ajax
...
urlpatterns = [
  ...
  url(r'^ics/(\d+)/([^/]+)/(my-classes).ics$',classes.ics_classes_user,
      name='ics_classes_user'),
  ...
  url(r'^delay_reschedule/(\d+)/(\d|close)/$', classes.delay_reschedule,
      name='delay_reschedule'),
