...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for course in orm['course.Course'].objects.all():
            sections = orm['course.Section'].objects.filter(course=course)
            if not sections.count():
                continue
            section = sections[0]
            if sections.count() > 1:
                print "using %s instead of:"%section.term.start
                for s in sections:
                    print s.term.start
            for field in SYNC_FIELDS:
                setattr(course,field,getattr(section,field))
            course.save()
        for session in orm['course.Session'].objects.all():
            session.course = session.section.course
            session.save()
