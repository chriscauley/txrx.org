...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        session_type = orm['contenttypes.contenttype'].objects.get(name='session',app_label='course')
        course_type = orm['contenttypes.contenttype'].objects.get(name='course',app_label='course')
        from mptt_comments.models import MpttComment
        comments = MpttComment.objects.filter(content_type=session_type)
        print "Changing %s comments"%len(comments)
        for comment in comments:
            course = comment.content_object.course
            comment.content_type_id = course_type.id
            comment.object_pk = course.pk
            comment.save()
        print "%s comments missed"%MpttComment.objects.filter(content_type=session_type)
