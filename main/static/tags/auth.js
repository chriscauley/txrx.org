riot.tag('login', '<form name="form" onsubmit="{ submit }"> <input-field each="{ fields }"></input-field> <div class="submit-row"> <input type="submit" value="Log in" class="btn btn-info"> <a href="/auth/password_reset/" class="pull-right">Forgotten username/password?</a> <div class="alert alert-danger error" if="{ non_field_errors }">{ non_field_errors }</div> </div> </form>', function(opts) {
