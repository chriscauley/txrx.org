...
riot.tag2('course-list', '<div each="{opts.courses}" if="{visible}"> <a href="{url}" class="course well {well_class}" id="course_{id}"> <div class="picture"> <img riot-src="{im.url}" width="{im.width}" height="{im.height}"> <div class="enrolled-status" status="{enrolled_status}"></div> </div> <div class="details"> <div class="subjects" if="{TXRX.DEBUG}"><span each="{subject in subject_names}">{subject}</span></div> <div class="title">{name}</div> <div class="description">{short_description}</div> <div class="enrolled-status" data-status="{enrolled_status}"></div> <div class="sessions" if="{active_sessions.length}"> <span class="next_session {active_sessions[0].closed_status}">{active_sessions[0].short_dates}</span> <div class="pull-right"> <span class="full_sessions" if="{full_sessions.length}"> [{full_sessions.length} Full<span class=" hidden-xs"> Session{(full_sessions.length > 1)?⁗s⁗:⁗⁗}</span>] </span> <span class="open_sessions" if="{open_sessions.length}"> [{open_sessions.length} Open<span class=" hidden-xs"> Session{(open_sessions.length > 1)?⁗s⁗:⁗⁗}</span>] </span> </div> </div> </div> <div class="price">{(fee > 0 && next_time != 0)?(⁗$⁗+fee):⁗FREE⁗}</div> </a> </div>', '', '', function(opts) {
});
...
riot.tag2('event-occurrence', '<div class="well" name="ajax_target"> <a if="{uR.auth.user.is_superuser}" href="/event/orientations/{start_slug}/" class="admin-link fa fa-pencil-square"></a> <a if="{uR.auth.user.is_staff && parent.event.allow_rsvp}" href="/tools/master/event/rsvp/?object_id={id}" class="admin-link fa fa-check-square-o" style="top: auto; bottom: 0;"></a> <div class="dates"> <div>{start_string}</div> </div> <b class="full" if="{full}">This occurrence is full</b> <div if="{!past && parent.event.allow_rsvp}"> <div if="{!full && authenticated}"> <button class="btn btn-success rsvp" if="{!quantity}" onclick="{makeRSVP}">RSVP for this event</button> <button class="btn btn-danger unrsvp" if="{quantity}" onclick="{cancelRSVP}">Cancel RSVP</button> <span if="{total_rsvp && uR.auth.user.is_superuser}" data-reddot="{total_rsvp}"></span> </div> <div if="{!full && !authenticated}"> You must <a href="{uR.urls.auth.login}">Login</a> to RSVP </div> </div> <div if="{past}"> <span class="btn btn-warning">You can no longer RSVP</span> </div> </div>', '', '', function(opts) {
...
//# sourceMappingURL=txrx-built.js.map
